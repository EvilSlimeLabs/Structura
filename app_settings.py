...
## Where settings used to live. Read once, to carry an existing choice across,
...
def settings_file():
    ...
    A `.structura` sitting next to the executable wins, which makes the program
    portable: put it on a stick with its settings beside it and it carries them
    between machines, touching nothing on the host. Otherwise settings live in
    the home directory, where they survive replacing the executable and work
    when the program is run from a folder the user cannot write to. The home
    copy is the one created when neither exists.
    """
    portable = os.path.join(paths.beside_executable(), SETTINGS_NAME)
...
## the desktop cannot be asked -- a dark window is the safer thing to show
